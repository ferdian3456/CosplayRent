...
func (repository *UserRepositoryImpl) FindByUUID(ctx context.Context, tx *sql.Tx, uuid string) (user.UserResponse, error) {
	...
	query := "SELECT id,name,email,address,created_at,updated_at FROM users where id=$1"
	...
	if rows.Next() {
		err := rows.Scan(&users.Id, &users.Name, &users.Email, &users.Address, &createdAt, &updatedAt)
		helper.PanicIfError(err)

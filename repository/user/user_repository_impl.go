...
func (repository *UserRepositoryImpl) Update(ctx context.Context, tx *sql.Tx, user user.UserUpdateRequest) {
	query := "UPDATE users SET name=$2,email=$3,password=$4,address=$5,profile_picture=$6  WHERE id=$1"
	_, err := tx.ExecContext(ctx, query, user.Id, user.Name, user.Email, user.Password, user.Address, user.Profile_picture)
	helper.PanicIfError(err)
}

...
func (repository *ReviewRepositoryImpl) Create(ctx context.Context, tx *sql.Tx, review domain.Review) {
	...
	query := "INSERT INTO reviews (user_id,costume_id,description,review_picture,rating,created_at) VALUES ($1,$2,$3,$4,$5,$6)"
	_, err := tx.ExecContext(ctx, query, review.User_id, review.Costume_id, review.Description, review.Review_picture, review.Rating, review.Created_at)
...
func (repository *ReviewRepositoryImpl) FindUserReview(ctx context.Context, tx *sql.Tx, uuid string) ([]review.OwnReviewResponse, error) {
	...
	query := "SELECT id,user_id,costume_id, description, review_picture, created_at FROM reviews WHERE user_id=$1"
	...
	for rows.Next() {
		...
		err = rows.Scan(&review.Id, &review.Userid, &review.Costume_Id, &review.Description, &review.Review_picture, &review.Created_at)
		helper.PanicIfError(err)
		reviews = append(reviews, review)
		hasData = true
	}
...
func (repository *ReviewRepositoryImpl) FindUserReviewByReviewID(ctx context.Context, tx *sql.Tx, uuid string, reviewid int) (review.OwnReviewByReviewID, error) {
	...
	query := "SELECT user_id, costume_id, description,rating, created_at FROM reviews WHERE id=$1"
